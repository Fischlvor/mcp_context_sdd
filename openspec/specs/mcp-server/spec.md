...
Implements the MCP (Model Context Protocol) server layer: protocol handshake, tool discovery, the `search-libraries` and `get-library-docs` tools, call logging, and supporting operational endpoints (health check, Swagger UI).
